...
def plot_audio(audio_data, ax, label):
    ax.title.set_text('Áudio gravado')
    ax.plot(audio_data, label=label)
    ax.set_xlabel('Amostra')
    ax.set_ylabel('Amplitude')
    ax.grid()

def record_and_plot(duration=5, sample_rate=44100):
    audio_data = record_audio(duration, sample_rate)
    fig, ax = plt.subplots()
    plot_audio(audio_data, ax, 'Gravação')
    plt.tight_layout()
    plt.show()
